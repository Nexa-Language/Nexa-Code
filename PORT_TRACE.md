...
| `src/context.nx` : `get_git_status`/`get_claude_md`/`get_agents_md`/`get_current_date`/`get_env_info`/`build_user_context` | `src/context.ts:36-188`（getGitStatus/getSystemContext/getUserContext）+ `src/utils/claudemd.ts`（getClaudeMds 层级核心）+ `src/utils/autonomyAuthority.ts`（AGENTS.md workspace authority）+ `src/constants/prompts.ts:604-711`（computeSimpleEnvInfo/getShellInfoLine/getUnameSR）+ `common.ts`（getLocalISODate） | **partial** | 1b。git/日期逐字段对齐（含截断 1000+"2k characters"文案）；CLAUDE.md 层级移植 user+project(cwd 向上)，managed(/etc)+.claude/rules/*.md 暂缓；Anthropic 产品行(env 营销)省略注明。GLM 实跑：CLAUDE.md(5186 字符)+date 正确注入。**get_agents_md**：发现根 AGENTS.md + .claude/autonomy/AGENTS.md（cwd 向上，根->cwd 合并），注入 "Workspace authority from AGENTS.md"。bench 修复：memory 簇 task_223 由 FAIL→PASS（agent 读到 AGENTS.md 指令→写 MEMORY.md "- Город: Москва"，事件证据 Read×3/Write×3）。autonomy 触发/HEARTBEAT/调度暂缓(partial) |
