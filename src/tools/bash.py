...
__tool_Bash_schema = {
    "name": "Bash",
    "description": "Executes a bash command. You may specify an optional timeout in milliseconds (up to 600000ms / 10 minutes). By default, your command will timeout after 120000ms (2 minutes).",
    "parameters": {
        "type": "object",
        "properties": {
        "command": {
        ...
        "run_in_background": {
                "type": "string",
                "description": "Parameter run_in_background"
        }
},
        "required": ["command", "timeout", "run_in_background"]
    }
}

def Bash(command, timeout, run_in_background):
    """Executes a bash command. You may specify an optional timeout in milliseconds (up to 600000ms / 10 minutes). By default, your command will timeout after 120000ms (2 minutes)."""
    import os
    import shutil
    import subprocess
    ...
    def _run_cmd(_c, _t):
        if _bash:
            return subprocess.run([_bash, '-c', _c], capture_output=True, text=True, timeout=_t)
        return subprocess.run(_c, shell=True, capture_output=True, text=True, timeout=_t)
    ...
    if run_in_background:
        _bg_note = '\n[note: run_in_background requested; async background-task subsystem not ported — ran synchronously]'
    return '%s\n[exit code: %d]%s' % (_result, _code, _bg_note)
from src.runtime.tools_registry import LOCAL_TOOLS
LOCAL_TOOLS['Bash'] = Bash
