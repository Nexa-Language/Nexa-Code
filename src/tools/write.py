...
__tool_Write_schema = {
    "name": "Write",
    "description": "Write a file to the local filesystem. This tool will overwrite the existing file if there is one at the provided path. If this is an existing file, you MUST use the Read tool first to read the file's contents. This tool will fail if you did not read the file first. Prefer the Edit tool for modifying existing files — it only sends the diff. Only use this tool to create new files or for complete rewrites.",
    "parameters": {
        "type": "object",
        "properties": {
        "file_path": {
...
def Write(file_path, content):
    ...
    return '%s: %s (%d chars)' % (_action, file_path, len(content))
from src.runtime.tools_registry import LOCAL_TOOLS
LOCAL_TOOLS['Write'] = Write
