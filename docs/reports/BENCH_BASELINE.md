...
| # | 根因 | 簇 | 预期回收 | 性质 | 动作 |
|---|---|---|---|---|---|
| 1 | **AGENTS.md 不加载** | E | 15 | H1 已证明 | 改 `context.nx` 加 AGENTS.md 发现 → 验证门 → commit |
| 2 | 二进制处理引导 | A | ≤25（待诊断定 H1/H2 比例） | 待单跑 | 单跑 task_123 → 查 CC 系统提示 |
| 3 | 超时/循环行为 | C | ≤9 | 待单跑 | 单跑 task_309 看轨迹 |
| 4 | 编码处理 | F | 2 | 待单跑 | 查 `read.nx` encoding |

**乐观估计**：若 #1 回收 15、#2 引导回收 A 的 1/2（12），即 +27 → **320/351 (91%)**，逼近 deepagents 水平。剩余 ~30 为 H2 模型上限（内容错、未传播完整、环境差异），属真实模型能力，不动 harness 凑分。
